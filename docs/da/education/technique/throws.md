...
```mermaid
graph TD
    A[Petanque-kast] --> B[Peger]
    A --> C[Skydning]

    B --> D[Efter bane]
    B --> E[Af Spin]

    C --> F[Ved Impact Point]
    C --> G[Efter intensitet]
...
```mermaid
graph LR
    A[Pegebaner] --> B[Roulette <br/> Rullende]
    A --> C[Demiportée <br/> Halv lob]
    A --> D[Portée <br/> Lob]
...
```mermaid
graph LR
    A[Spinkontrol] --> B[Fuldt bagspin]
    A --> C[Halv spin]
    A --> D[Ingen spin]

    B --> E[Stopper/trækker tilbage]
    C --> F[Kontrolleret rulning]
    D --> G[Naturlig rulle]
...
```mermaid
graph TD
    A[Skydemetoder] --> B[Au fer <br/> Jernskud]
    A --> C[Devant <br/> Foran]
    A --> D[Sauteret <br/> Springskud]

    B --> E[Direkte træf]
    C --> F[Land før målet]
    D --> G[Hoppe ind i målet]
...
```mermaid
graph LR
    A[Lær det grundlæggende] --> B[Udvikl 2-3 pålidelige kast]
    B --> C[Teknisk fundament]
    C --> D[Indtast flowtilstand]
    D --> E[Kroppen vælger naturligt]
