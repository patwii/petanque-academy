...
```mermaid
graph LR
    A[Daglig praksis] --> B[Nervebaner]
    B --> C[Automatisk færdighed]
    C --> D[Konkurrencepræstation]
...
```mermaid
graph TD
    A[Konsekvent praksis] --> B[Uge 1: Opbygning af vaner]
    B --> C[Måned 1: Opsigelsesfordele]
    C --> D[Måned 3: Mere naturligt]
    D --> E[År 1+: Grundlæggende færdighed]
