...
```mermaid
graph LR
    A[Værkstedsrejse] --> B[IRL-møde]
    B --> C[Opbyg tillid]
    C --> D[Online sessioner]
    D --> E[Løbende støtte]
    E --> F[Fortsat vækst]
...
```mermaid
graph TD
    A[Workshopformat] --> B[Fase 1: Personligt møde]
    A --> C[Fase 2: Online]
    ...
    C --> G[Regelmæssige indtjekninger]
    C --> H[Fortsat deling]
    C --> I[Fleksibel planlægning]
...
```mermaid
graph TD
    A[Peer-læring] --> B[Delte oplevelser]
    A --> C[Flere perspektiver]
    A --> D[Gensidig støtte]

    B --> E[Du er ikke alene]
    C --> F[Nye indsigter]
    D --> G[Ansvarlighed]
