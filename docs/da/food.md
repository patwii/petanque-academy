...
```mermaid
graph TD
    A[Ernæringsstrategi] --> B[Stabilt blodsukker]
    A --> C[Protein og sunde fedtstoffer]
    A --> D[Hydrering]
...
```mermaid
graph LR
    A[Sukker/Simple kulhydrater] --> B[Spike]
    B --> C[Krak]
    C --> D[Rystelser og angst]

    E[Protein og fedtstoffer] --> F[Stabil]
    F --> G[Konsekvent ydeevne]
...
```mermaid
graph TD
    A[Spis sukker] --> B[Blodsukkerstigninger]
