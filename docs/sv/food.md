...
```mermaid
graph TD
    A[Näringsstrategi] --> B[Stabilt blodsocker]
    A --> C[Protein och hälsosamma fetter]
    A --> D[Hydrering]
...
```mermaid
graph LR
    A[Socker/enkla kolhydrater] --> B[Spika]
    B --> C[Krascha]
    C --> D[Skakningar och ångest]

    E[Protein och fetter] --> F[Stabil]
    F --> G[Konsekvent prestanda]
...
```mermaid
graph TD
    A[Ät socker] --> B[Blodsockerspikar]
    B --> C[Insulinfrisättning]
    C --> D[Blodsockerkrascher]
