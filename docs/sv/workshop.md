...
- **Nybörjare (2-3 timmar):** Introduktion till mentala spelkoncept → [Se guiden för den mentala resan](/sv/mental-journey/sessionsguide)
...
| **Relaterade guider** | Andra träningsformat | [Mental resa](/sv/mental-resa/) • [Träningsläger](/sv/träningsläger) |
...
```mermaid
graph LR
    A[Workshopsession] --> B[Grundläggning 60 min]
    B --> C[Djupgående arbete 90 min]
    C --> D[Integration 45 min]
    D --> E[Stänger 15 min]
...
```mermaid
graph TD
    A[Fysiskt utrymme] --> B[Signalerar mental förändring]
    A --> C[Inga hinder]
    A --> D[Lika positionering]

    B --> E[Psykologisk säkerhet]
    C --> E
    D --> E
...
```mermaid
graph TD
    A[Incheckningsmatris] --> B[Hög energi + positiv]
    A --> C[Hög energi + negativ]
    A --> D[Låg energi + Positiv]
    A --> E[Låg energi + negativ]

    B --> F[Erkänn alla stater]
    C --> F
    D --> F
    E --> F
...
```mermaid
graph TD
    A[Rädsla i en hatt] --> B[Anonym delning]
    B --> C[Peer-validering]
    C --> D[Skam upplöses]
    D --> E[Isoleringsbrytningar]
    E --> F[Psykologisk säkerhet]
