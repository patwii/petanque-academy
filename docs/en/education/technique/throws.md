...
```mermaid
graph LR
    A[Pointing Trajectories] --> B[Roulette<br/>Rolling]
    A --> C[Demi-portée<br/>Half-lob]
    A --> D[Portée<br/>Lob]
...
```mermaid
graph LR
    A[Spin Control] --> B[Full Backspin]
    A --> C[Half Spin]
    A --> D[No Spin]

    B --> E[Stops/Pulls Back]
    C --> F[Controlled Roll]
    D --> G[Natural Roll]
...
```mermaid
graph TD
    A[Shooting Methods] --> B[Au fer<br/>Iron shot]
    A --> C[Devant<br/>In front]
    A --> D[Sautée<br/>Jump shot]

    B --> E[Direct hit]
    C --> F[Land before target]
    D --> G[Bounce into target]
...
```mermaid
graph LR
    A[Learn Basics] --> B[Develop 2-3 Reliable Throws]
    B --> C[Technical Foundation]
    C --> D[Enter Flow State]
    D --> E[Body Chooses Naturally]
...
- **[The Zone](/en/education/mental-game/the-zone/)** - Accessing flow states consistently
- **[Training Methods](/en/education/technique/training/)** - How to practice effectively
- **[Mental Strength](/en/education/mental-game/mental-strength/)** - Performing under pressure
:::
