...
```mermaid
graph TD
    A[Flow State] --> B[Challenge/Skill Balance]
    A --> C[Clear Goals]
    A --> D[Immediate Feedback]
    A --> E[Total Focus]
    A --> F[Loss of Self-Consciousness]
    A --> G[Sense of Control]
...
```mermaid
graph LR
    A[Assessment<br/>Outside Circle] --> B[Transition<br/>Enter Circle]
    B --> C[Physical Setup<br/>In Circle]
    C --> D[Visualization<br/>2-3 seconds]
    D --> E[Execution<br/>Throw]
...
|------------|---------------------|------------------|
| **Internal** ❌ | Your body mechanics | "Keep my elbow straight"<br/>"Follow through properly"<br/>"Don't grip too tight" |
| **External** ✅ | Target and outcome | "Land it right there"<br/>"See the path"<br/>"Hit the target" |
...
```mermaid
graph LR
    A[Slow Breath] --> B[Activates Calm System]
    B --> C[Lower Heart Rate]
    B --> D[Relaxed Muscles]
    B --> E[Clear Mind]
...
```mermaid
graph LR
    A[Mistake] --> B[STOP<br/>Pause]
    B --> C[OBSERVE<br/>Notice]
    C --> D[ACCEPT<br/>It happened]
    D --> E[SLIP<br/>Let go]
