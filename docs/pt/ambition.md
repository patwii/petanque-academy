...
```mermaid
graph LR
    A[Treinamento tradicional] --> B[Foco técnico]
    B --> C[Platô]

    D[Pétanque Academy] --> E[Foco no estado de fluxo]
    E --> F[Desempenho de nível superior]
...
```mermaid
graph TD
    A[Jogador de Elite] --> B[Domínio técnico]
    B --> C{What's Next?}
