...
```mermaid
graph TD
    A[Prática de Mindfulness] --> B[Práticas formais]
    A --> C[Práticas informais]
    A --> D[Específico para a competição]

    B --> E[Meditação sentada]
    B --> F[Escaneamento corporal]
    B --> G[Meditação caminhando]

    C --> H[Respiração Consciente]
    C --> I[Método SOAS]
    C --> J[Alimentação Consciente]

    D --> K[Hálito antes da injeção]
    D --> L[Reinicialização entre lançamentos]
    D --> M[Sino da Atenção Plena]
