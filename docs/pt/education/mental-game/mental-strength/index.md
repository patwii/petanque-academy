...
```mermaid
graph TD
    A[Situação de pressão] --> B{Mental Strength?}
    B -->|Weak| C[Pensamentos negativos]
    B -->|Strong| D[Rotina pré-filmagem]

    C --> E[A tensão aumenta]
    E --> F[Queda de desempenho]
    F --> G[Mais pressão]
    G --> C

    D --> H[Foque no processo]
    H --> I[Treinamento de confiança]
    I --> J[Executar bem]
    J --> K[A confiança aumenta.]
...
```mermaid
graph LR
    A[Melhor preparação] --> B[Mais confiança]
    B --> C[Melhor desempenho]
    C --> D[Mais confiança]
    D --> A
...
- **[Lidando com a Pressão](/en/education/mental-game/mental-strength/handling-pressure)** - Técnicas para situações de alto risco
- **[Rotina Pré-Arremesso](/en/education/mental-game/mental-strength/pre-shot-routine)** - Construindo seu gatilho de desempenho
