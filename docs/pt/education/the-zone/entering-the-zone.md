...
```mermaid
graph TD
    A[Estado de fluxo] --> B[Equilíbrio entre desafio e habilidade]
    A --> C[Objetivos claros]
    A --> D[Feedback imediato]
    A --> E[Foco total]
    A --> F[Perda da autoconsciência]
    A --> G[Senso de controle]
...
```mermaid
graph LR
    A[Avaliação <br/> Círculo externo] --> B[Transição <br/> Entre no círculo]
    B --> C[Configuração física <br/> Em círculo]
    C --> D[Visualização <br/> 2-3 segundos]
    D --> E[Execução <br/> Lançar]
...
|------------|---------------------|------------------|
| **Interno** ❌ | A mecânica do seu corpo | &quot;Mantenha meu cotovelo reto&quot;<br/> &quot;Dê continuidade ao processo adequadamente&quot;<br/> &quot;Não aperte com muita força&quot; |
| **Externo** ✅ | Meta e resultado | &quot;Aterre bem aí&quot;<br/> &quot;Veja o caminho&quot;<br/> &quot;Acerte o alvo&quot; |
...
```mermaid
graph LR
    A[Respiração lenta] --> B[Ativa o sistema de calma]
    B --> C[Diminuir a frequência cardíaca]
    B --> D[Músculos Relaxados]
    B --> E[Mente Clara]
...
```mermaid
graph LR
    A[Erro] --> B[PARAR <br/> Pausa]
    B --> C[OBSERVAR <br/> Perceber]
    C --> D[ACEITAR <br/> Aconteceu]
    D --> E[ESCORREGAR <br/> Solte]
