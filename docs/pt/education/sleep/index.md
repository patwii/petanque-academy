...
```mermaid
flowchart TD
    A[Qualidade do sono] --> B[Firmeza das mãos]
    A --> C[Tomando uma decisão]
    A --> D[Controle Emocional]
    A --> E[Recuperação física]

    B --> F[Precisão]
    C --> F
    D --> F
    E --> G[Consistência ao longo do tempo]

    F --> H[Desempenho na partida]
    G --> H
