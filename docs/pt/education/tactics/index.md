...
```mermaid
graph TD
    A[Sua vez] --> B{Are you holding?}
    B -->|Yes| C{Opponent has boules?}
    B -->|No| D{Can you point closer?}

    C -->|Yes| E[Espere e planeje]
    C -->|No| F{Safe to add points?}

    F -->|Yes| G[Adicione os pontos com cuidado.]
    F -->|No| H[Jogue com segurança/bloqueie]

    D -->|Yes, high %| I[Apontar]
    D -->|Talvez| J{Can you shoot?}
    D -->|No| K[Mova o macaco ou o bloco.]
...
```mermaid
graph LR
    A[Distância] --> B[6-7m Curto]
    A --> C[7-9m Médio]
    A --> D[9-11m de comprimento]
...
- **[Decisões Baseadas em Probabilidade](/pt/education/tactics/probability)** - Usando a matemática para fazer escolhas melhores
