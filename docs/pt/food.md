...
```mermaid
graph TD
    A[Estratégia de Nutrição] --> B[Níveis estáveis de açúcar no sangue]
    A --> C[Proteínas e gorduras saudáveis]
    A --> D[Hidratação]

    B --> E[Foco nítido]
    B --> F[Mãos firmes]
    B --> G[Acalme os nervos]

    C --> E
    C --> F

    D --> E
    D --> H[Desempenho Físico]
...
```mermaid
graph LR
    A[Açúcar/Carboidratos Simples] --> B[Espinho]
    B --> C[Colidir]
    C --> D[Tremores e Ansiedade]

    E[Proteínas e Gorduras] --> F[Estável]
    F --> G[Desempenho consistente]
...
```mermaid
graph TD
    A[Coma açúcar] --> B[Picos de açúcar no sangue]
    B --> C[Insulina liberada]
    C --> D[Quedas bruscas de açúcar no sangue]
    D --> E[Resposta da Adrenalina]
    E --> F[Tremores]
    E --> G[Ansiedade]
    E --> H[Foco insuficiente]
