...
```mermaid
flowchart TD
    A[Momento de Alto Risco] --> B[Resposta de luta ou fuga]
    B --> C[Frequência cardíaca ↑]
    B --> D[Respiração superficial]
    B --> E[Tensão muscular]
    B --> F[Foco restrito]
    ...
    G -->|Threat Mode| H[Desempenho ↓]
    G -->|Challenge Mode| I[Desempenho ↑]
