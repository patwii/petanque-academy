...
```mermaid
flowchart TD
    A[Escolhas nutricionais] --> B[Estabilidade do açúcar no sangue]
    B --> C[Função cognitiva]
    B --> D[Controle motor fino]
    B --> E[Regulação Emocional]

    C --> F[Decisões Acertadas]
    D --> G[Mãos firmes]
    E --> H[Calma sob pressão]

    F --> I[Desempenho máximo]
    G --> I
    H --> I
