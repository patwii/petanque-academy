...
```mermaid
graph LR
    A[Sesión de taller] --> B[Fundación 60min]
    B --> C[Trabajo profundo 90min]
    C --> D[Integración 45min]
    D --> E[Cierre 15min]
...
```mermaid
graph TD
    A[Espacio físico] --> B[Señales de cambio mental]
    A --> C[Sin barreras]
    A --> D[Posicionamiento igualitario]

    B --> E[Seguridad psicológica]
    C --> E
    D --> E
...
```mermaid
graph TD
    A[Matriz de registro] --> B[Alta energía + Positiva]
    A --> C[Alta energía + Negativa]
    A --> D[Baja energía + Positiva]
    A --> E[Baja energía + Negativo]

    B --> F[Reconocer a todos los estados]
    C --> F
    D --> F
    E --> F
...
```mermaid
graph TD
    A[Miedo en un sombrero] --> B[Compartir de forma anónima]
    B --> C[Validación por pares]
    C --> D[La vergüenza se disuelve]
    D --> E[Pausas de aislamiento]
    E --> F[Seguridad psicológica]
