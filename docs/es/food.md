...
graph TD
    ...
    B --> E[Enfoque nítido]
    B --> F[Manos firmes]
    B --> G[Calmar los nervios]

    C --> E
    C --> F

    D --> E
    D --> H[Rendimiento físico]
...
```mermaid
graph LR
    A[Azúcar/Carbohidratos simples] --> B[Espiga]
    B --> C[Chocar]
    C --> D[Temblores y ansiedad]

    E[Proteínas y grasas] --> F[Estable]
    F --> G[Rendimiento consistente]
...
```mermaid
graph TD
    A[Come azúcar] --> B[picos de azúcar en la sangre]
    B --> C[Insulina liberada]
    C --> D[Caídas del azúcar en la sangre]
    D --> E[Respuesta de adrenalina]
    E --> F[Temblores]
    E --> G[Ansiedad]
    E --> H[Mala concentración]
