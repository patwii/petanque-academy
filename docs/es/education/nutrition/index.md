...
```mermaid
graph LR
    A[Azúcar/Carbohidratos simples] --> B[pico de azúcar en la sangre]
    B --> C[Liberación de insulina]
    C --> D[Caída del azúcar en la sangre]
    D --> E[Respuesta de adrenalina]
    E --> F[Temblores y ansiedad]

    G[Proteínas y grasas saludables] --> H[Nivel de azúcar en sangre estable]
    H --> I[Energía constante]
    I --> J[Enfoque nítido]
    J --> K[Manos firmes]
...
```mermaid
graph TD
    A[Dieta actual] --> B[Cambios graduales]
    B --> C[Adaptación corporal <br/> 2-4 semanas]
    C --> D[Nueva normalidad]
    D --> E[Listo para la competencia]

    F[Cambio repentino <br/> Día de la competición] --> G[Malestar digestivo]
    G --> H[Bajo rendimiento]
