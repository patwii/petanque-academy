...
```mermaid
graph TD
    A[Lanzamientos de petanca] --> B[Señalando]
    A --> C[Tiroteo]

    B --> D[Por trayectoria]
    B --> E[Por Spin]

    C --> F[Por punto de impacto]
    C --> G[Por intensidad]
...
```mermaid
graph LR
    A[Trayectorias de apuntamiento] --> B[Ruleta <br/> Laminación]
    A --> C[Demi-portée <br/> Medio globo]
    A --> D[Portée <br/> Lob]
    A --> E[Plombée <br/> Tiro de caída]
...
```mermaid
graph LR
    A[Control de giro] --> B[Retroceso completo]
    A --> C[Medio giro]
    A --> D[Sin giro]

    B --> E[Se detiene/retrocede]
    C --> F[Rollo controlado]
    D --> G[Rollo natural]
...
```mermaid
graph TD
    A[Métodos de disparo] --> B[En el fer <br/> Perdigones de hierro]
    A --> C[Devant <br/> Al frente]
    A --> D[Salteado <br/> Tiro de salto]

    B --> E[Impacto directo]
    C --> F[Aterriza antes del objetivo]
    D --> G[Rebotar hacia el objetivo]
...
```mermaid
graph LR
    A[Intensidad del disparo] --> B[Duro y plano]
    A --> C[Medio duro]
    A --> D[Arco alto suave]

    B --> E[Potencia + Velocidad]
    C --> F[Balance]
    D --> G[Precisión]
...
```mermaid
graph LR
    A[Aprenda los conceptos básicos] --> B[Desarrolla 2-3 lanzamientos confiables]
    B --> C[Fundación Técnica]
    C --> D[Entrar en estado de flujo]
    D --> E[El cuerpo elige naturalmente]
...
- **[La Zona](/es/education/mental-game/the-zone/)** - Acceder a estados de flujo de forma consistente
- **[Métodos de entrenamiento](/es/education/entrenamiento/)** - Cómo practicar eficazmente
- **[Fuerza mental](/es/education/mental-game/mental-strength/)** - Rendimiento bajo presión
:::
