...
```mermaid
graph TD
    A[Situación de presión] --> B{Mental Strength?}
    B -->|Weak| C[Pensamientos negativos]
    B -->|Strong| D[Rutina previa al disparo]

    C --> E[La tensión aumenta]
    E --> F[Disminución del rendimiento]
    F --> G[Más presión]
    G --> C

    D --> H[Centrarse en el proceso]
    H --> I[Formación en confianza]
    I --> J[Ejecutar bien]
    J --> K[La confianza crece]
...
```mermaid
graph LR
    A[Mejor preparación] --> B[Más confianza]
    B --> C[Mejor rendimiento]
    C --> D[Más confianza]
    D --> A
