...
```mermaid
graph LR
    A[Pekebaner] --> B[Rulett <br/> Rulling]
...
```mermaid
graph LR
    A[Spinnkontroll] --> B[Full bakspinn]
    A --> C[Halv spinn]
    A --> D[Ingen spinn]

    B --> E[Stopper/trekker tilbake]
    C --> F[Kontrollert rulling]
    D --> G[Naturlig rull]
...
```mermaid
graph TD
    A[Skytemetoder] --> B[Au fer <br/> Jernskudd]
    A --> C[Devant <br/> Foran]
    A --> D[Sautert <br/> Hoppskudd]

    B --> E[Direkte treff]
    C --> F[Land før målet]
    D --> G[Sprette inn i målet]
...
```mermaid
graph LR
    A[Skuddintensitet] --> B[Hardt, flatt]
    A --> C[Middels hard]
    A --> D[Myk høy bue]
...
```mermaid
graph LR
    A[Lær det grunnleggende] --> B[Utvikle 2–3 pålitelige kast]
    B --> C[Teknisk fundament]
    C --> D[Angi flyttilstand]
    D --> E[Kroppen velger naturlig]
