...
```mermaid
graph TD
    A[Jouw beurt] --> B{Are you holding?}
    B -->|Yes| C{Opponent has boules?}
    B -->|No| D{Can you point closer?}

    C -->|Yes| E[Wacht en plan]
    C -->|No| F{Safe to add points?}

    F -->|Yes| G[Tel de punten zorgvuldig op.]
    F -->|No| H[Speel op veilig/blokkeer]

    D -->|Yes, high %| I[Punt]
    D -->|Misschien| J{Can you shoot?}
    D -->|No| K[Verplaats krik of blok]
...
- **[Op waarschijnlijkheid gebaseerde beslissingen](/en/education/tactics/probability)** - Wiskunde gebruiken om betere keuzes te maken
