...
```mermaid
graph TD
    A[Voedingsstrategie] --> B[Stabiele bloedsuikerspiegel]
    A --> C[Eiwitten en gezonde vetten]
    A --> D[Hydratatie]

    B --> E[Scherpe focus]
    B --> F[Vaste handen]
    B --> G[Zenuwen kalmeren]

    C --> E
    C --> F

    D --> E
    D --> H[Fysieke prestaties]
...
```mermaid
graph LR
    A[Suiker/eenvoudige koolhydraten] --> B[Spike]
    B --> C[Botsing]
    C --> D[Tremoren en angst]

    E[Eiwitten en vetten] --> F[Stabiel]
    F --> G[Constante prestaties]
...
```mermaid
graph TD
    A[Eet suiker] --> B[Bloedsuikerspiegelpieken]
    B --> C[Insuline vrijgegeven]
    C --> D[Bloedsuikerdalingen]
    D --> E[Adrenalinereactie]
    E --> F[Aardbevingen]
    E --> G[Spanning]
    E --> H[Slechte focus]
