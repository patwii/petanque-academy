...
- **Beginners (2-3 uur):** Introductie tot mentale spelconcepten → [Zie Mentale Reisgids](/en/mental-journey/session-guide)
...
| **Gerelateerde handleidingen** | Andere trainingsvormen | [Mentale Reis](/en/mental-journey/) • [Trainingskamp](/en/training-camp) |
...
```mermaid
graph LR
    A[Workshopsessie] --> B[Basiscursus 60 min.]
    B --> C[Intensief werk van 90 minuten]
    C --> D[Integratie 45 min.]
    D --> E[Sluitingstijd: 15 minuten]
...
```mermaid
graph TD
    A[Fysieke ruimte] --> B[Signalen van een mentale omslag]
    A --> C[Geen barrières]
    A --> D[Gelijke positionering]

    B --> E[Psychologische veiligheid]
    C --> E
    D --> E
...
```mermaid
graph TD
    A[Incheckmatrix] --> B[Veel energie + positiviteit]
    A --> C[Hoge energie + negatief]
    A --> D[Lage energie + positief]
    A --> E[Lage energie + negatief]

    B --> F[Erken alle staten]
    C --> F
    D --> F
    E --> F
...
```mermaid
graph TD
    A[Angst in een hoed] --> B[Anoniem delen]
    B --> C[Peervalidatie]
    C --> D[Schaamte verdwijnt]
    D --> E[Isolatiepauzes]
    E --> F[Psychologische veiligheid]
