...
```mermaid
graph TD
    A[Pétanque-worpen] --> B[Wijzen]
    A --> C[Schieten]

    B --> D[Via traject]
    B --> E[Door Spin]

    C --> F[Door Impact Point]
    C --> G[Qua intensiteit]
...
```mermaid
graph LR
    A[Richtingstrajecten] --> B[Roulette <br/> Rollend]
    A --> C[Demi-portée <br/> Halflob]
    A --> D[Portée <br/> Lob]
...
```mermaid
graph LR
    A[Spin Control] --> B[Volledige backspin]
    A --> C[Halve draai]
    A --> D[Niet draaien]

    B --> E[Stopt/trekt terug]
    C --> F[Gecontroleerde rol]
    D --> G[Natuurlijke rol]
...
```mermaid
graph TD
    A[Opnamemethoden] --> B[Au fer <br/> IJzeren kogels]
    A --> C[Voorkant <br/> Vooraan]
    A --> D[Sautée <br/> Sprongschot]

    B --> E[Voltreffer]
    C --> F[Landen vóór het doel]
    D --> G[Stuiter richting het doel]
...
```mermaid
graph LR
    A[Schotintensiteit] --> B[Hard Flat]
    A --> C[Middelhard]
    A --> D[Zachte hoge boog]
...
- **[De Zone](/nl/education/the-zone/)** - Consistent toegang krijgen tot flow-toestanden
- **[Trainingsmethoden](/nl/education/training/)** - Hoe effectief te oefenen
- **[Mentale kracht](/nl/education/mental-strength/)** - Presteren onder druk
:::
