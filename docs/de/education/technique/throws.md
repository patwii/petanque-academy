...
```mermaid
graph TD
    A[Boule-Würfe] --> B[Zeigen]
    A --> C[Schießen]

    B --> D[Nach Flugbahn]
    B --> E[Von Spin]

    C --> F[Nach Aufprallpunkt]
    C --> G[Nach Intensität]
...
```mermaid
graph LR
    A[Zeigebahnen] --> B[Roulette <br/> Rollend]
    A --> C[Demi-portée <br/> Halblob]
    A --> D[Portée <br/> Lob]
...
```mermaid
graph LR
    A[Spin Control] --> B[Voller Rückwärtsspin]
    A --> C[Halbe Drehung]
    A --> D[Kein Spin]

    B --> E[Stoppt/Zieht zurück]
    C --> F[Kontrollierte Rollbewegung]
    D --> G[Natural Roll]
...
```mermaid
graph TD
    A[Aufnahmemethoden] --> B[Au fer <br/> Eisenkugel]
    A --> C[Devant <br/> Vorne]
    A --> D[Sautiert <br/> Sprungwurf]

    B --> E[Volltreffer]
    C --> F[Landen Sie vor dem Ziel]
    D --> G[Abpraller zum Ziel]
...
```mermaid
graph LR
    A[Grundlagen lernen] --> B[Entwickle 2-3 zuverlässige Würfe]
    B --> C[Technisches Fundament]
    C --> D[Flow-Zustand aktivieren]
    D --> E[Der Körper wählt auf natürliche Weise]
...
- **[The Zone](/en/education/mental-game/the-zone/)** – Flow-Zustände dauerhaft erreichen
- **[Trainingsmethoden](/en/education/technique/training/)** – Wie man effektiv übt
- **[Mentale Stärke](/en/education/mental-game/mental-strength/)** – Leistung unter Druck
:::
