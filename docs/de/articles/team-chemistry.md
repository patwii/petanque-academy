...
```mermaid
flowchart TD
    A[Teamchemie] --> B[Vertrauen]
    A --> C[Verständnis]
    A --> D[Kommunikation]
    A --> E[Unterstützung]
    A --> F[Vergnügen]

    B --> G[Vertrauen in die eigenen Fähigkeiten]
    C --> H[Wissen, wie Teamkollegen denken]
    D --> I[Effektiver Informationsfluss]
    E --> J[Gegenseitige Ermutigung]
    F --> K[Freude am gemeinsamen Spielen]
