...
- **Anfänger (2-3 Std.):** Einführung in die Konzepte des mentalen Trainings → [Siehe Mental Journey Guide](/en/mental-journey/session-guide)
...
| **Verwandte Leitfäden** | Andere Trainingsformate | [Mentale Reise](/en/mental-journey/) • [Trainingslager](/en/training-camp) |
...
```mermaid
graph LR
    A[Workshop-Sitzung] --> B[Grundlagen 60 Minuten]
    B --> C[Tiefenarbeit 90 Minuten]
    C --> D[Integration 45 Minuten]
    D --> E[Schließung 15 Minuten]
...
```mermaid
graph TD
    A[Physischer Raum] --> B[Signale für einen mentalen Wandel]
    A --> C[Keine Barrieren]
    A --> D[Gleiche Positionierung]

    B --> E[Psychologische Sicherheit]
    C --> E
    D --> E
...
```mermaid
graph TD
    A[Check-in-Matrix] --> B[Hohe Energie + Positiv]
    A --> C[Hohe Energie + Negativ]
    A --> D[Niedrige Energie + Positiv]
    A --> E[Niedrige Energie + Negativ]

    B --> F[Anerkennung aller Staaten]
    C --> F
    D --> F
    E --> F
...
```mermaid
graph TD
    A[Angst im Hut] --> B[Anonymes Teilen]
    B --> C[Peer-Validierung]
    C --> D[Scham löst sich auf]
    D --> E[Isolationspausen]
    E --> F[Psychologische Sicherheit]
