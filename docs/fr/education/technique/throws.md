...
```mermaid
graph TD
    A[Lancers de pétanque] --> B[Pointer]
    A --> C[Tournage]

    B --> D[Par trajectoire]
    B --> E[Par Spin]

    C --> F[Par point d&#39;impact]
    C --> G[Par intensité]
...
```mermaid
graph LR
    A[Trajectoires de pointage] --> B[Roulette <br/> Roulement]
    A --> C[Demi-portée <br/> Demi-lob]
    A --> D[Portée <br/> Lob]
...
```mermaid
graph LR
    A[Contrôle de rotation] --> B[Retournement complet]
    A --> C[Demi-tour]
    A --> D[Sans rotation]
...
```mermaid
graph TD
    A[Méthodes de prise de vue] --> B[Au fer <br/> Grenaille de fer]
    A --> C[Devant <br/> Devant]
    A --> D[Sautée <br/> tir en suspension]

    B --> E[Coup direct]
    C --> F[Atterrissez avant la cible]
    D --> G[Rebondir vers la cible]
...
```mermaid
graph LR
    A[Intensité du tir] --> B[Plat dur]
    A --> C[Moyennement dur]
    A --> D[Arche haute et souple]

    B --> E[Puissance + Vitesse]
    C --> F[Équilibre]
    D --> G[Précision]
...
- **[La Zone](/en/education/mental-game/the-zone/)** - Accéder de manière constante à des états de flow
- **[Méthodes de formation](/en/education/technique/training/)** - Comment s&#39;entraîner efficacement
- **[Force mentale](/en/education/mental-game/mental-strength/)** - Performance sous pression
:::
