...
```mermaid
graph TD
    A[Situation de pression] --> B{Your Response}
    B -->|Anxiety| C[Baisse des performances]
    B -->|Excitement| D[Les performances augmentent]

    C --> E[Pensées négatives]
    C --> F[Tension]
    C --> G[Mauvaise mise au point]
