...
```mermaid
graph TD
    A[État de flux] --> B[Équilibre défi/compétences]
    A --> C[Objectifs clairs]
    A --> D[Réponse immédiate]
    A --> E[Concentration totale]
    A --> F[Perte de conscience de soi]
    A --> G[Sentiment de contrôle]
...
```mermaid
graph LR
    A[Évaluation <br/> Cercle extérieur] --> B[Transition <br/> Entrez dans le cercle]
    B --> C[configuration physique <br/> Dans le cercle]
    C --> D[Visualisation <br/> 2-3 secondes]
    D --> E[Exécution <br/> Lancer]
...
|------------|---------------------|------------------|
| **Interne** ❌ | La mécanique de votre corps | « Garder mon coude droit »<br/> &quot;Assurer le suivi correctement&quot;<br/> « Ne serrez pas trop fort » |
...
```mermaid
graph LR
    A[Respiration lente] --> B[Active le système Calme]
    B --> C[Diminution du rythme cardiaque]
    B --> D[Muscles détendus]
    B --> E[Esprit clair]
...
```mermaid
graph LR
    A[Erreur] --> B[ARRÊT <br/> Pause]
    B --> C[OBSERVER <br/> Avis]
    C --> D[ACCEPTER <br/> C&#39;est arrivé]
    D --> E[GLISSER <br/> Lâcher]
    E --> F[Retour au présent]
