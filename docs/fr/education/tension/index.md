...
```mermaid
flowchart TD
    A[Tension] --> B[Pression de la poignée ↑]
    A --> C[raideur des épaules]
    A --> D[Mouvement précipité]
    A --> E[Respiration superficielle]

    B --> F[Sensation et contrôle ↓]
    C --> G[Mouvement des bras restreint]
    D --> H[Le timing est détruit]
    E --> I[Concentration ↓]
