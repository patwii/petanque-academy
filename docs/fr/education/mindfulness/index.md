...
```mermaid
graph TD
    A[Une pensée surgit] --> B{Notice it?}
    B -->|No - Autopilot| C[Se laisser prendre au piège de la pensée]
    B -->|Yes - Mindful| D[Observer sans jugement]

    C --> E[Réaction émotionnelle]
    E --> F[Les performances en pâtissent]

    D --> G[Laisse tomber]
    G --> H[Retour au présent]
    H --> I[Restez concentré]
...
graph LR
    ...
    B --> C[OBSERVER]
    C --> D[ACCEPTER]
    D --> E[GLISSER/Lâcher prise]
    E --> F[Retour au présent]
    F --> G[Prêt pour le prochain lancer]
