...
```mermaid
graph TD
    A[Stratégie nutritionnelle] --> B[Glycémie stable]
    A --> C[Protéines et graisses saines]
    A --> D[Hydratation]

    B --> E[Mise au point nette]
    B --> F[Mains fermes]
    B --> G[Calmer les nerfs]

    C --> E
    C --> F

    D --> E
    D --> H[Performance physique]
...
```mermaid
graph LR
    A[Sucres/Glucides simples] --> B[Pointe]
    B --> C[Accident]
    C --> D[Tremblements et anxiété]

    E[Protéines et lipides] --> F[Écurie]
    F --> G[Performance constante]
...
```mermaid
graph TD
    A[Consommez du sucre] --> B[pics de glycémie]
    B --> C[Libération d&#39;insuline]
    C --> D[Chutes de glycémie]
    D --> E[Réponse à l&#39;adrénaline]
    E --> F[Tremblements]
    E --> G[Anxiété]
    E --> H[Mauvaise mise au point]
