...
```mermaid
flowchart TD
    A[Un revers survient] --> B{Resilient?}

    B -->|Yes| C[Accepter → Apprendre → Réinitialiser]
    B -->|No| D[Frustration → Tension → Plus d&#39;erreurs]

    C --> E[Rétablissement rapide]
    D --> F[Dégrincer]

    E --> G[Performance maintenue]
    F --> H[Effondrement des performances]
