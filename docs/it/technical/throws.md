...
```mermaid
graph LR
    A[Controllo dello spin] --> B[Backspin completo]
    A --> C[Mezzo giro]
    A --> D[Nessuna rotazione]

    B --> E[Si ferma/si tira indietro]
    C --> F[Rollio controllato]
    D --> G[Rotolo naturale]
...
```mermaid
graph TD
    A[Metodi di ripresa] --> B[Al ferro <br/> Pallini di ferro]
    A --> C[Avanti <br/> Davanti]
    A --> D[Saltare in padella <br/> Tiro in sospensione]

    B --> E[Colpo diretto]
    C --> F[Atterrare prima dell&#39;obiettivo]
    D --> G[Rimbalzare sul bersaglio]
...
```mermaid
graph LR
    A[Intensità del tiro] --> B[Duro e piatto]
    A --> C[Medio-duro]
    A --> D[Arco morbido alto]

    B --> E[Potenza + Velocità]
    C --> F[Bilancia]
    D --> G[Precisione]
...
```mermaid
graph LR
    A[Impara le basi] --> B[Sviluppa 2-3 lanci affidabili]
    B --> C[Fondazione tecnica]
    C --> D[Entra nello stato di flusso]
    D --> E[Il corpo sceglie naturalmente]
