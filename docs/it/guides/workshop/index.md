...
| **Guide correlate** | Altri formati di formazione | [Viaggio mentale](/it/guides/mental-journey/) • [Campo di addestramento](/it/guides/training-camp/) |
...
```mermaid
graph LR
    A[Sessione di workshop] --> B[Fondotinta 60 min]
    B --> C[Lavoro profondo 90 min]
    C --> D[Integrazione 45 min]
    D --> E[Chiusura 15min]
...
```mermaid
graph TD
    A[Spazio fisico] --> B[Segnali di cambiamento mentale]
    A --> C[Nessuna barriera]
    A --> D[Posizionamento paritario]

    B --> E[Sicurezza psicologica]
    C --> E
    D --> E
...
```mermaid
graph TD
    A[Matrice di check-in] --> B[Alta energia + positivo]
    A --> C[Alta energia + negativo]
    A --> D[Bassa energia + positivo]
    A --> E[Bassa energia + negativo]

    B --> F[Riconosci tutti gli stati]
    C --> F
    D --> F
    E --> F
...
```mermaid
graph TD
    A[La paura nel cappello] --> B[Condivisione anonima]
    B --> C[Validazione tra pari]
    C --> D[La vergogna si dissolve]
    D --> E[Pause di isolamento]
    E --> F[Sicurezza psicologica]
