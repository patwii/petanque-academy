...
```mermaid
graph TD
    A[Situazione di pressione] --> B{Mental Strength?}
    B -->|Weak| C[Pensieri negativi]
    B -->|Strong| D[Routine pre-tiro]

    C --> E[La tensione aumenta]
    E --> F[Calo delle prestazioni]
    F --> G[Più pressione]
    G --> C

    D --> H[Concentrarsi sul processo]
    H --> I[Formazione sulla fiducia]
    I --> J[Eseguire bene]
    J --> K[La fiducia cresce]
...
```mermaid
graph LR
    A[Migliore preparazione] --> B[Più fiducia]
    B --> C[Prestazioni migliori]
    C --> D[Più fiducia]
    D --> A
