...
```mermaid
graph TD
    A[Il pensiero nasce] --> B{Notice it?}
    B -->|No - Autopilot| C[Lasciati catturare dai pensieri]
    B -->|Yes - Mindful| D[Osservare senza giudizio]

    C --> E[Reazione emotiva]
    E --> F[Le prestazioni ne risentono]

    D --> G[Lasciarlo andare]
    G --> H[Ritorno al presente]
    H --> I[Rimani concentrato]
...
graph LR
    ...
    B --> C[OSSERVARE]
    C --> D[ACCETTARE]
    D --> E[SLIP/Lascia andare]
    E --> F[Ritorno al presente]
    F --> G[Pronti per il prossimo lancio]
