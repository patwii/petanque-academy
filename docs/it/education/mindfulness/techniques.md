...
```mermaid
graph TD
    A[Pratica di consapevolezza] --> B[Pratiche formali]
    A --> C[Pratiche informali]
    A --> D[Specifico per la competizione]

    B --> E[Meditazione seduta]
    B --> F[Scansione corporea]
    B --> G[Meditazione camminata]

    C --> H[Respirazione consapevole]
    C --> I[Metodo SOAS]
    C --> J[Mangiare consapevolmente]

    D --> K[Respiro pre-tiro]
    D --> L[Reset tra lanci]
    D --> M[Campana della consapevolezza]
